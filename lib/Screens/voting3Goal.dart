...
class Voting3Goal extends StatefulWidget {
  final Practice currentPractice;
  const Voting3Goal({super.key, required this.currentPractice});

  @override
  State<StatefulWidget> createState() => Voting3GoalState();
}

class Voting3GoalState extends State<Voting3Goal> {
  int currentIndex = -1;
  List<UserClient> teamA = [];
  List<UserClient> teamB = [];
  UserClient? selectedBestGoal;
  ...
  void onPlayerTap(UserClient player) {
    setState(() {
      ...
      if (selectedBestGoal == player) {
        selectedBestGoal = null; // Avmarkera
      } else {
        selectedBestGoal = player; // Markera ny spelare
      }
    });
  }
                  ...
                  return Stack(
                    children: [
                      ...
                      ...TeamFormation(
                        ...
                        selectedBestGoal: selectedBestGoal,
                      ).buildFormation(),
                      ...TeamFormation(
                        team: teamB,
                        isTeamA: false,
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      Award? goalAward =
                          awards
                              .where((award) => award.type == Type.GOAL)
                              .firstOrNull;

                      if (selectedBestGoal != null && goalAward != null) {
                        String result = await _backendPC.updateVote(
                          playerToVoteOn: selectedBestGoal!,
                          awardId: goalAward.id!,
                          votedFor: true,
                        );
                    ...
                    child: Text(
                      selectedBestGoal != null
                          ? 'Skicka in'
                          : 'Skippa och skicka in',
                    ),
                  ),
                ],
              ),
...
class TeamFormation {
  ...
  final UserClient? selectedBestGoal;
  ...
  TeamFormation({
    ...
    required this.selectedBestGoal,
  });
